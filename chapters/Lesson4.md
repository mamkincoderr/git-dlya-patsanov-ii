...
## ⬇️ Получаем изменения — fetch vs pull

Коллега запушил изменения на GitHub. Тебе надо их забрать. Есть два способа, и разница важная.
...
### Отменить уже запушенный коммит — `git revert`

`git reset` ломает историю когда коммит уже запушен. Для этого случая есть `git revert` — он не удаляет коммит, а создаёт новый который его отменяет. История остаётся честной, коллеги не страдают.
...
> *Правило простое: не запушил — можно `reset`. Уже запушил — только `revert`.*
...
> ⚠️ **Важно:** `.gitignore` работает только для файлов которые Git ещё не трекает. Если файл уже был закоммичен раньше — просто добавить его в `.gitignore` не поможет, Git продолжит его видеть. Нужно сначала убрать его из трекинга:
